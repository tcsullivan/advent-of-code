...
create hist 100 cells allot hist 100 0 cfill
...
: roll-north            loads width @ height @ cfill
                        ...
                        j i round? if j i height @ i load@ - move-y i reduce then
                        j i cube? if height @ j - 1- i setpot then
                        loop loop ;
: roll-west             loads height @ width @ cfill
                        width @ 0 do height @ 0 do
                        i j round? if i j width @ i load@ - move-x i reduce then
                        i j cube? if width @ j - 1- i setpot then
                        loop loop ;
: roll-south            loads width @ height @ 1- cfill
                        0 height @ 1- do width @ 0 do
                        j i round? if j i i load@ move-y i reduce then
                        j i cube? if j 1- i setpot then
                        loop -1 +loop ;
: roll-east             loads height @ width @ 1- cfill
                        0 width @ 1- do height @ 0 do
                        i j round? if i j i load@ move-x i reduce then
                        i j cube? if j 1- i setpot then
                        loop -1 +loop ;
: cycle                 roll-north roll-west roll-south roll-east ;
: score                 0 pad ! height @ 0 do width @ 0 do
                        j i round? if height @ j - pad +! then
                        loop loop pad @ ;
...
: score-target          200 1 do cycle loop
                        100 0 do cycle score hist i cells + ! loop
                        90 10 do hist dup i cells + 10 memcmp
                        if i leave then loop
                        target 300 - dup rot mod - 300 +
